...
document.addEventListener("DOMContentLoaded", async function() {
  ...
  try{
    if(leagueTable){
      const ClubKey = prompt("League key(152 207 206 153 175 168(ligue1) 302)", "168");
      (async (leagueKey) => {
  ...
  try{
    if(clubTable){
      const LeagueKey = prompt("League key(100,80)", "100");
      (async (clubKey) => {
        ...
        clubTable.innerHTML = `
          <tr class="information__tr-main">
            ...
            <th class="information__th">Number</th>
            <th class="information__th age">Age</th>
            <th class="information__th">Matches</th>
            <th class="information__th goals">Goals</th>
            <th class="information__th">Yellow Card</th>
            <th class="information__th">Red Card</th>
        ...
        informationClub.result[0].players.forEach((item, i) => {
          ...
          <tr class="information__tr">
            ...
            <td class="information__td">${item.player_number}</td>
            <td class="information__td age">${item.player_age}</td>
            <td class="information__td">${item.player_match_played}</td>
            <td class="information__td goals">${item.player_goals}</td>
            <td class="information__td">${item.player_yellow_cards}</td>
            <td class="information__td">${item.player_red_cards}</td>
            <td class="information__td">${item.player_type}</td>
          </tr>
          `;
        });
      })(LeagueKey)
